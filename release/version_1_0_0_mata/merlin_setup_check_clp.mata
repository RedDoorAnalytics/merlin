...
void merlin_cmp_errorchecks(`gml' gml,`SS' dv)
{
	...
	//check each component, allow only one of each type
	pos 	= 1	
	tind	= 0
	varind 	= 0
	reind 	= 0
...
void merlin_dvcheck(`gml' gml,`SS' dv2, `RS' tind, `RS' varind, reind)
{
	...
	if (strpos(dv2,".")) merlin_error("Stata's . notation (e.g. c. or i.) in variable definitions is not currently supported")
	...
	if (pos1 & pos2) { 															//random effect or ?EV[outcome]
		...
		hasfn 	= hasrcs | hasfp | hasmf
		if (!hasfn) mjcerror("Invalid function "+dv2)
		...
		if (hasfp) {															//fp syntax check
			...
			if (st_local("offset")!="" & st_local("moffset")!="") {
				mjcerror("offset() and moffset() can't both be specified")
			}
			
			powers 	= strtoreal(tokens(st_local("powers")))						//check fp powers
			nfp 	= cols(powers)
			if (nfp>2) mjcerror("Invalid "+dv2)
			
			allpows = (-2,-1,-0.5,0,0.5,1,2,3)
			for (i=1;i<=nfp;i++) {
				if (!sum(powers[1,i]:==allpows)) mjcerror("Invalid "+dv2)
			}
			...
			stata("syntax varlist(min=1 max=1) , [DF(string) KNOTS(numlist asc) LOG ORTHog EVent OFFset(varname) MOFFset(varname)]")
			...
			if (st_local("df")!="") {
				if (st_local("knots")!="") {
					mjcerror("Cannot specify both df() and knots() within rcs()")
				}
				df = strtoreal(st_local("df"))
				if (df<1 | df>10) 	mjcerror("df() must be between 1 and 10")
				if (df!=round(df)) 	mjcerror("df() must be an integer")
			}
			else {
				if (st_local("knots")=="") {
					mjcerror("One of df() or knots() needed in rcs()")
				}
			}
...
void merlin_confirm_depvar(`gml' gml,dv,pos1,pos2)	
{
	...
	resp = tokens(st_local("response"+strofreal(gml.model)))[1]		//accounts for survival
	...
	for (i=1;i<=gml.Nmodels;i++) {
		if (i!=gml.model) {
			resp = tokens(st_local("response"+strofreal(i)))[1]		//accounts for survival
			if (yvar==resp | yvar==strofreal(i)) {
				flag 	= 0
				k 		= i
			}
		}
