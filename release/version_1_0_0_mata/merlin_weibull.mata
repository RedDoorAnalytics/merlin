...
`RM' merlin_logl_weibull(`gml' gml)
{
	...
	if (gml.nobhaz[gml.model]) {
		if (gml.NI[gml.model]) 	{
			result = y[,2] :* logh :- merlin_weibull_ch_ni(gml,gam,y[,1])
		}
		else {
			result = y[,2] :* logh :- merlin_weibull_ch(xzb,gam,y[,1]) 
			if (gml.hasltrunc[gml.model]) result = result :+ merlin_weibull_ch(merlin_util_xzb(gml,y[,3]),gam,y[,3])
		}
	}
	else {
		if (gml.NI[gml.model]) {
			...
			if (gml.hasltrunc[gml.model]) result = result :+ merlin_weibull_ch(merlin_util_xzb(gml,y[,3]),gam,y[,1])
		}
	}
