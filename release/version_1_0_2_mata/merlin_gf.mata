				...
				`RM' H)
{
	...
	if (gml.Nrelevels) 	{
		...
		if (gml.hasanyltrunc) lnfi = lnfi :- merlin_ltrunc(gml)
		
		gml.Pgml->lnfi1 = lnfi
	}
	else lnfi = merlin_logl_ob(gml)												//logl ob level
							...
							`Sgml' gml)		//	
{
	...
	if (index<gml.Nrelevels) {
		...
		res2 = (*gml.ltruncP[gml.model])(gml)
		if (gml.hasweights[index2])	res2 = res2 :* asarray(gml.weights,(index2,j))
		res = panelsum(res2,asarray(gml.panelindexes,(index,j)))	
	}
