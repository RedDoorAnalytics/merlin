...
{synopt :{opt f:ailure}}failure probability{p_end}
...
{synopt :{opt rmft}}restricted mean time since failure{p_end}
...
{phang} 
{cmd:rmft} calculates the restricted mean time since failure within the interval (0,tau], which is the integral of 
the cumulative incidence function.

{phang} 
{cmd:cif} calculates the cumulative incidence function for competing risks models. 
All survival models in the fitted {cmd:merlin} model are assumed to be cause-specific 
event time models contributing to the calculation. When only one survival model is 
included in the {cmd:merlin} model, the {cmd:cif} and {cmd:failure} options are equivalent. 
