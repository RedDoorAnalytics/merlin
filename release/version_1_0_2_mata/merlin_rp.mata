...
`RM' merlin_logl_rp(`gml' gml)
{
	...
	if (gml.nobhaz[mod]) {
		return(y[,2] :* (logch :+ log(drcs :+ merlin_util_xzb_deriv(gml,y[,1]))) :- exp(logch))
	}
	else {
		return(y[,2] :* log(exp(logch :+ log(drcs :+ merlin_util_xzb_deriv(gml,y[,1]))):/y[,1] :+ asarray(gml.bhazards,mod)) :- exp(logch))
	}
}
