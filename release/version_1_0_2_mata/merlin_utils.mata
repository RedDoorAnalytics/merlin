...
`RM' merlin_util_xzb(`GMLS' gml, | `RC' t)
{
	...
	for (c=1;c<=Ncmps;c++) {
		...
		bi = bi + Nbs			//update index

	}

	if (gml.hascons[mod]) xb = xb :+ b[bi++] 		//intercept
	return(xb)	
}
...
`RM' merlin_util_xzb_deriv(`GMLS' gml, | `RC' t)
{
	...
	hstep 	= 1e-8 :* (abs(tvar) :+ 1e-8)
	lh 		= merlin_util_xzb(gml,tvar :+ hstep:/2)
	rh 		= merlin_util_xzb(gml,tvar :- hstep:/2)
	return((lh :- rh):/(hstep))
}
...
`RM' merlin_util_expval_deriv(`GMLS' gml, | `RC' t)
{
	...
	hstep 	= 1e-8 :* (abs(tvar) :+ 1e-8)
	lh 		= merlin_util_expval(gml,tvar :+ hstep:/2)
	rh 		= merlin_util_expval(gml,tvar :- hstep:/2)
	return((lh :- rh):/(hstep))
}
