...
void merlin_predict_error_check(`gml' gml, `SS' stat)
{
	...
	if (!issurv) {
		if (stat=="hazard" | stat=="survival" | stat=="chazard" | stat=="rmst" |  stat=="rmft" | stat=="failure" | stat=="cif" | stat=="rmftcif") {
...
`RM' merlin_p_cif(`gml' gml, | `RC' t)
{
	...
	//pointer to cause-specific hazard function -> gml.model indexes this
	...
	for (mod=1; mod<=gml.Nmodels; mod++) {
		...
		if (issurv & mod!=refmod) {
			hfs 	= hfs,merlin_p_getpf(gml, "hazard")
	...
	for (q=1; q<=Ngq; q++) {						//cif integral
		...
		osqp 		= qp[,q] :/ 2 :* J(gml.N,1,gq[,1]') :+ qp[,q] :/ 2
		ochres 		= J(gml.N,1,0)
		
		for (p=1; p<=Ngq; p++) {					//overall cumulative hazard integral
			for (k=1; k<=Nsurvmodels; k++) {
				gml.model = modind[k]
				ochres = ochres :+ (*hfs[k])(gml,osqp[,p]) :* gq[p,2] :* qp[,q] :/ 2
			}
		}
...
`RM' merlin_p_rmftcif(`gml' gml, | `RC' t)
{
	not = args()==1
	if (not) t = merlin_util_timevar(gml)
	Nqp = 30
	gq 	= merlin_gq(Nqp,"legendre")
	qp	= t :/ 2 :* J(gml.N,1,gq[,1]') :+ t:/2
