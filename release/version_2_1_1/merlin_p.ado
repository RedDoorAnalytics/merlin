			...
			if "`e(family1)'"=="cox" {
				if "`STAT'"!="basehazard" {
					forvalues i=1/`e(Nmodels)' {
						tempvar baseh`i'
						if "`passbaseh'"=="" {
							qui predict `baseh`i'' if `touse', basehazard outcome(`i')
						}
						else {
		...
		//====================================================================================================================//
			...
			//====================================================================================================================//
				...
				if  "`e(family1)'"!="cox" {
					...
					else if ("`STAT'"=="cif" | "`STAT'"=="chazard" | "`STAT'"=="rmst" | "`STAT'"=="sdifference" | "`STAT'"=="cifdifference" | "`STAT'"=="rmstdifference") {
				...
				if "`e(family1)'"!="cox" {
					...
					if "`STAT'"=="eta" {
						...
						forvalues r = 1/`reps' {
							...
							predict _merlin_cox, 	`STAT' `xbtype' at(`at') at1(`at1') at2(`at2') 		///
													timevar(`timevar') ltruncated(`ltruncated')			///
													`globalopts'
