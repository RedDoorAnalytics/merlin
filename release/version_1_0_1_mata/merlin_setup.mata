...
// [op] - denotes an option which is not always completely filled in
	...
	`RC' hasltrunc		//[op]	//has left truncation/delayed entry
	...
	`RC' xbindex				//ob. level index for each outcome, based on main touse core index
	...
	`RC' Ncmps					//number of specifications in each complex syntax
	`TR' elindex
	`TR' Nels
	`RM' eqnindex
	`RC' hascons
	...
	//for predictions
	`RS' predict				//leave struct behind for predictions
	`RS' fixedonly				//predictions based only on fixed effects
...
void merlin_setup(`SS' GML,`SS' touse)
{
	...
	if (gml.Nrelevels & sum(gml.hasltrunc)) merlin_error("ltruncated() not allowed with a multilevel model\n")
	...
	//initialise integation
	gml.updateMC = st_local("updatemc")!=""
	merlin_init_numint(gml)
	merlin_hazNI_init(gml)
	if (gml.Nlevels>1) merlin_init_integration(gml, levelvars, gml.modeltouses)
	...
	st_local("Nres",strofreal(sum(gml.Nres)))
...
void merlin_get_n_dist_ancp(`gml' gml)
{
	...
	for (i=1;i<=gml.Nmodels;i++) {
		...
		else if (familys[i]=="rp" | familys[i]=="rcs") {
...
void merlin_get_xb_p(`gml' gml)
{
	...
	for (i=1;i<=nm;i++) {
		...
		else if (familys[i]=="rcs" | familys[i]=="rp") {
...
void merlin_get_logl_p(`gml' gml)
{
	...
	for (i=1;i<=nm;i++) {
		...
		if (familys[i]=="exponential") {
			...
			else 										pmat[i] = &merlin_surv_weibull()
		}
		else if (familys[i]=="gompertz") {
...
// 				gq = gq,( 0.022935322010529,0.022935322010529,0.063092092629979,0.063092092629979,0.104790010322250,0.104790010322250,0.140653259715525,0.140653259715525,0.169004726639267,0.169004726639267,0.190350578064785,0.190350578064785,0.204432940075298,0.204432940075298,0.209482141084728)'
				...
				//need response var stime, and if del entry
				ys = asarray(gml.y,i)
				if (gml.hasltrunc[i]) {
					asarray(gml.haznodes,i, (ys[,1]:-ys[,3]) :* J(gml.Nobs[gml.Nlevels,i],1,gq[,1]'):/2 :+ (ys[,1] :+ ys[,3]):/2)
					asarray(gml.hazweights,i,(ys[,1] :- ys[,3]) :* J(gml.Nobs[gml.Nlevels,i],1,gq[,2]'):/2)
				}
				else {
...
void merlin_init_numint(`gml' gml)
{
	...
	for (i=1;i<=gml.Nmodels;i++) {
		if (familys[i]=="exponential" | familys[i]=="weibull" | familys[i]=="gompertz" | (familys[i]=="user" & st_local("hazfunction"+strofreal(i))!="") | (familys[i]=="user" & st_local("loghazfunction"+strofreal(i))!="") | (familys[i]=="user" & st_local("failure"+strofreal(i))!="")) {
			Ncmps = asarray(gml.Nels,i)
			
			for (j=1;j<=gml.Ncmps[i];j++) {

				//check for &t functions
				elindex = asarray(gml.elindex,(i,j))	
				if (sum(3:==elindex) | sum(8:==elindex) | sum(9:==elindex)) gml.NI[i] = 1

				//check for EV or XB
				if (sum(4:==elindex) | sum(5:==elindex) | sum(6:==elindex) | sum(7:==elindex) | sum(10:==elindex) | sum(11:==elindex) | sum(12:==elindex) | sum(13:==elindex)) {
					for (k=1;k<=Ncmps[j];k++) {
						if (elindex[k]==4 | elindex[k]==5 | elindex[k]==6 | elindex[k]==7 | elindex[k]==10 | elindex[k]==11 | elindex[k]==12 | elindex[k]==13) {
							index = asarray(gml.elinfo,(i,j,k))
							for (p=1;p<=gml.Ncmps[index];p++) {
								//check for &t functions
								elindex2 = asarray(gml.elindex,(index,p))	
								if (sum(3:==elindex2) | sum(8:==elindex2) | sum(9:==elindex2)) gml.NI[i] = 1
							}
						}
					}
				}
