...
`RM' merlin_logl_exp(`gml' gml)
{
	...
	if (gml.nobhaz[gml.model]) {
		if (gml.NI[gml.model]) 	{
			...
			if (gml.hasltrunc[gml.model]) result = result :+ merlin_exp_ch(merlin_util_xzb(gml,y[,3]),y[,3])
		}
	}
	else {
		if (gml.NI[gml.model]) {
			...
			if (gml.hasltrunc[gml.model]) result = result :+ merlin_exp_ch(merlin_util_xzb(gml,y[,3]),y[,1])
		}
	}
	return(result)
}
