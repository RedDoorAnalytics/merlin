...
void merlin_build_cmps(`gml' gml)
{
	...
	EV[] 						- expected value of outcome								
	dEV[] 						- first derivative of expected value of outcome			
	d2EV[] 						- second derivative of expected value of outcome		
	iEV[]						- integral of expected value of outcome over t			
	XB[] 						- expected value of CP									
	dXB[] 						- first derivative of expected value of CP				
	d2XB[] 						- second derivative of expected value of CP				
	iXB[]						- integral of expected value of CP over t				
	gp()						- gaussian process										
	*/
	...
	for (mod=1;mod<=gml.Nmodels;mod++) {
		...
		asarray(gml.CmpBIndex,mod,cmpbindex)
		...
		if (gml.familys[mod]=="ordinal") {
			...
			for (dap=1;dap<=gml.Ndistancp[mod];dap++) {
				...
				if (gml.familys[mod]=="rp" | gml.familys[mod]=="prp") {
					if (dap==1) gml.initdistapindex = gml.initdistapindex,(eqn\1)
					else 		gml.initdistapindex = gml.initdistapindex,(eqn\0)
				}
...
`RS' merlin_build_els(`gml' gml, `RS' mod, `RS' i, `RS' Nels, `SS' dv2)
{
	...
	if (hassquareb & !hasroundb) {
		...
		else if (strpos(el,"bs(")) 	return(merlin_setup_bs(gml,mod,i,Nels,el))			//bs function
		else if (strpos(el,"ps(")) 	return(merlin_setup_ps(gml,mod,i,Nels,el))			//ps function
		else if (strpos(el,"gp(")) 	return(merlin_setup_gp(gml,mod,i,Nels,el))			//gp function
...
void merlin_build_variables(`gml' gml, `RS' mod, `RS' i, `RC' eltype, `RS' eqn, | `SS' at)
{
	...
	for (el=1;el<=Nels; el++) {
		...
		if 		(eltype[el]==1)	{
		...
		else if (eltype[el]==3 | eltype[el]==16) {								//mf() | gp()
			...
			nextvars = merlin_xz_bs(gml,i,el)

		}
		else if (eltype[el]==15) {												//ps()

			nextvars = merlin_xz_ps(gml,i,el)
	...
	if (!gml.nogen) {
		...
		build = sum(eltype:==3) | sum(eltype:==8) | sum(eltype:==9) | sum(eltype:==14) | sum(eltype:==15)
		if (build) {
...
`SS' merlin_cmp_label(`SS' el)
{
	...
	if (hassquareb) {
		...
		else if (strpos(el,"ps(")) 	return("ps()")			//ps function
		else if (strpos(el,"gp(")) 	return("gp()")			//gp function
