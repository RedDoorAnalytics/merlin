				...
				`RM' H)
{
	...
	if (gml.Nrelevels) 	{
		...
		if (gml.haspenalty[1]) 	{
			lnfi = lnfi :- merlin_global_penalty(gml,b):/gml.Nobs[1,1]			//overall penalty
		}
		...
		if (gml.Npenals)		lnfi = lnfi :- merlin_penalise_ob(gml,b)		//ps()
		
		if (gml.haspenalty[1]) 	lnfi = lnfi :- merlin_global_penalty(gml,b)		//overall penalty
