...
`RM' merlin_xz_bs(`GMLS' gml, `RS' c, `RS' el, | `RC' t)
{
	hast		= args()==4
	rcsinfo 	= asarray(gml.elinfo,(gml.model,c,el))
	...
	hasanyoffset 	= asarray(rcsinfo,6)
	if (hasanyoffset) rcsvar = rcsvar :+ asarray(rcsinfo,7)[index]
...
// 	len		= exp(asarray(gml.gpb,(gml.model,1)))
// 	sd1 	= exp(asarray(gml.gpb,(gml.model,2)))
