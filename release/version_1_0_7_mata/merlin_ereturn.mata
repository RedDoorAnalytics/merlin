...
void merlin_ereturn(`SS' GML)
{
	...
	if 		(gml.E.merlin==3) 	title = "Artificial neural network"
	...
	if (gml.haspenalty[1]) {
		if (gml.haspenalty[2]==1) 	stata("ereturn local penalty Lasso")
		else 						stata("ereturn local penalty Ridge")
		stata("ereturn local lambda "+strofreal(gml.haspenalty[3]))
	}
	...
	if (gml.Ncmps[mod]) {
		for (i=1;i<=gml.Ncmps[mod];i++) {
			...
			for (j=1;j<=Nels[i];j++) {
				...
				if (elindex[j]==14 | elindex[j]==15) {	//bs()/ps()
					index 			= strofreal(mod)+"_"+strofreal(i)+"_"+strofreal(j)
					rcsinfo 		= asarray(gml.elinfo,(mod,i,j))
					knots			= asarray(rcsinfo,3)			
