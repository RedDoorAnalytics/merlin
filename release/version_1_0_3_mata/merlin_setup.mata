...
// 		if (debug) printf("weights done\n")
	...
	if ((gml.hasanyltrunc | sum(gml.hasweights)) & strtoreal(st_local("devcode"))!=385927 & !gml.predict) {
		merlin_error("Nope")
	}
...
// void merlin_get_ltruncated(`gml' gml)
// {
// 	coreindex 	= st_local("coreindex")
// 	gml.lt 		= asarray_create("real",2)		//i,1 lt time, i,2 ltindex
// 	for (i=1;i<=gml.Nmodels;i++) {
		
// 		lt 		= st_data(.,st_local("ltruncated"+strofreal(i)),gml.modeltouses[1,i])
// 		ltindex = select(asarray(gml.xbindex,i),lt:>0)
// 		lt		= select(lt:>0)
// 		asarray(gml.lt,(i,1),lt)
// 		asarray(gml.lt,(i,2),ltindex)
// 	}
// }
