...
void merlin_init_ip_ltrunc(`gml' gml, levelvars, modeltouses)
{
		...
		if (gml.usegh[i]) {
			...
			if (gml.Nres[i]) {
				...
				if (gml.adapt[i]) {
					...
					asarray(gml.stackednodes_lt,i,asarray(gml.stackednodes,i))
					for (r=1;r<=gml.Nres[i];r++) {
						asarray(gml.aghip2_lt,(i,r),asarray(gml.aghip2,(i,r)))
					}	
					for (j=1; j<=gml.Nobs[i,1]; j++) {
						asarray(gml.aghip_lt,(i,j),asarray(gml.aghip,(i,j)))
					}
					asarray(gml.aghlogl_lt,i,asarray(gml.aghlogl,i))
