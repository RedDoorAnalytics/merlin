...
{p 8 12 2}
[{depvar}] [{it:component1}] [{it:component2}] [...] [, {it:{help merlin_models##model_options:model_options}}]
...
{synopt :{cmd: rcs [, {it:{help merlin_models##survival:survival}} {it:{help merlin_models##rcsopts:rcsopts}}]}}restricted cubic splines of log time on the log-hazard scale{p_end}
{synopt :{cmd: rp [, {it:{help merlin_models##survival:survival}} {it:{help merlin_models##rpopts:rpopts}}]}}Royston-Parmar{p_end}
...
{synopt :{opt knots(knots_list)}}internal knot locations for the baseline log cumulative hazard function{p_end}
...
{synoptset 25}{...}
{marker rpopts}{...}
{synopthdr:rcsopts - family(rcs)}
{synoptline}
{synopt :{opth df(#)}}degrees of freedom for the baseline log hazard function{p_end}
{synopt :{opt knots(knots_list)}}internal knot locations for the baseline log hazard function{p_end}
{synoptline}
{p2colreset}{...}
...
{synopt :{cmd: fp(}{it:varname}, {help merlin_models##fpopts:{it:fp_opts}}{cmd:)}}a fractional polynomial function of time{p_end}
{synopt :{cmd: rcs(}{it:varname}, {help merlin_models##rcsopts:{it:rcs_opts}}{cmd:)}}a restricted cubic spline function of time or log time{p_end}
...
{synopt :{opt knots(knots_list)}}knot locations for the spline function{p_end}
...
{phang2}{cmd:rcs(}{it:varname}, {help merlin_models##rcsopts:{it:rcs_opts}}{cmd:)} a restricted cubic spline function of, where {it:rcs_opts} are:
...
{phang}{opth failure(varname)} specifies the failure event. Should be coded 0 for censored observations, and 1 for events.
...
{phang}{opt knots(knots_list)} defines the internal knot locations for the spline functions used to model the baseline 
log cumulative hazard function

{dlgtab:RCS options}

{phang}{opt df(#)} degrees of freedom for the baseline log hazard function, i.e. number of restricted cubic 
spline terms.
