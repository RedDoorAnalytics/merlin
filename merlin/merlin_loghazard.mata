...
/*
-> functions for family(loghazard, ...)
*/


`RM' merlin_logl_loghazard(	`gml' gml, 	///
							`RM' G, 	///
							`RM' H)
{	
	model 		= gml.model
	...
	// log likelihood
		...
		if (merlin_get_nobs(gml,model)) {
			...
			logl[index1,] 	= merlin_util_xzb(gml) 
			if (hasbh[1]) {
				expxb1			= exp(logl[index1,])
				logl[index1,] 	= log(expxb1 :+ merlin_util_bhazard(gml))
			}
		...
		if (merlin_get_nobs(gml,model)) {
			...
			for (q=1;q<=Ngq;q++) {
				ch[,q] = exp(merlin_util_xzb(gml,qp[,q]))
			}
	...
	// score
		...
		if (merlin_get_nobs(gml,model)) {
			...
			if (hasbh[1]) {
				totalh1				= expxb1 :+ merlin_util_bhazard(gml)
				G[index1,sindex] 	= G[index1,sindex] :* expxb1 :/ totalh1
			}
		}
		...
		if (merlin_get_nobs(gml,model)) {
			...
			for (q=1;q<=Ngq;q++) {
				dch = dch :+ ch[,q] :* qw[,q] :* merlin_util_xz(gml,qp[,q])
			}
	...
	// hessian
		...
		if (merlin_get_nobs(gml,model)) {
			if (hasbh[1]) {
				Hx[index1,] = x[index1,xindex1] :* x[index1,xindex2] :* expxb1 :/ totalh1 :* (1 :- expxb1 :/ totalh1)
			}
		}
		...
		if (merlin_get_nobs(gml,model)) {
			...
			for (q=1;q<=Ngq;q++) {
				...
				dch = dch :+ ch[,q] :* qw[,q] :* x[,xindex1] :* x[,xindex2]
			}
...
`RM' merlin_loghazard_logh(`gml' gml, `RC' t)
{
...
`RM' merlin_loghazard_ch(`gml' gml, `RC' t, | `RC' t0)
{
	nobs 	= rows(t)
	ch 		= J(nobs,1,0)
	Ngq 	= gml.chip
	...
	if (args()==2) {
		for (q=1;q<=Ngq;q++) {
			ch = ch :+ exp(merlin_util_xzb(gml,qp[,q])) :* qw[,q]
		}
	}
	else {
		for (q=1;q<=Ngq;q++) {
			ch = ch :+ exp(merlin_util_xzb(gml,qp[,q],t0)) :* qw[,q]
		}
	}
	return(ch)
}

`RM' merlin_loghazard_s(`gml' gml, `RC' t)
{
	return(exp(-merlin_loghazard_ch(gml,t)))
}

`RM' merlin_loghazard_cdf(`gml' gml, `RC' t)
{
