...
`RM' merlin_p_exp_logh(`gml' gml, | `RC' t)
{	
	not = args()==1
...
`RM' merlin_p_exp_logch(`gml' gml, | `RC' t)
{	
	not = args()==1
	if (not) t = merlin_util_timevar(gml)
	...
	else return(merlin_util_xzb(gml,t) :+ log(t) )
}
...
`RM' merlin_p_exp_dens(`gml' gml, | `RC' t)
{	
	not = args()==1
	if (not) t = merlin_util_timevar(gml)
	return(merlin_p_exp_h(gml,t):*merlin_p_exp_s(gml,t))
}
