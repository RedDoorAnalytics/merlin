...
void merlin_xb_0(`GMLS' gml, `RR' b, `RS' eqnind)
{
	//
}
...
{
// 	dapmodelix = asarray(gml.dapmodels,gml.model)
// 	if (dapmodelix) {
// 		asarray(gml.distancb,(gml.model,1),exp(merlin_util_xzb_mod(gml,dapmodelix)))
// 	}
// 	else {
		eqnind = gml.eqnindex[gml.model,2] + 1
...
void merlin_xb_ggamma(`GMLS' gml, `RR' b, `RS' eqnind)
{
	eqnind = gml.eqnindex[gml.model,2] + 1
	asarray(gml.distancb,(gml.model,1),exp(b[eqnind++]))
	asarray(gml.distancb,(gml.model,2),b[eqnind++])
}
...
void merlin_xb_aft(`GMLS' gml, `RR' b, `RS' eqnind)
{
	mod 	= gml.model
	eqnind 	= gml.eqnindex[mod,2] + 1
	asarray(gml.distancb,(mod,1),b[|eqnind\(eqnind+gml.Ndistancp[mod]-2)|]')		//spline coefficients
	asarray(gml.distancb,(mod,2),b[1,eqnind+gml.Ndistancp[mod]-1])					//basis intercept
	eqnind = eqnind + gml.Ndistancp[mod]
}
...
void merlin_xb_pwexp(`GMLS' gml, `RR' b, `RS' eqnind)
{
	mod 	= gml.model
	eqnind 	= gml.eqnindex[mod,2] + 1
	Ncuts	= gml.Ndistancp[mod]
...
void merlin_xb_gp(`GMLS' gml, `RR' b, `RS' eqnind)
{
	eqnind 	= gml.eqnindex[gml.model,2] + 1
	Ndap	= gml.Ndistancp[gml.model]
	for (k=1; k<=Ndap; k++) asarray(gml.distancb,(gml.model,k),b[eqnind++])
}
