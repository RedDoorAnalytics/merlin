...
void merlin_init_integration(`gml' gml)
{	
...
void merlin_init_ip(`gml' gml)
{
	...
	if (sum(gml.adapt)) {
		...
		for (j=1;j<=gml.Nmodels;j++) {
			ids = st_data(.,gml.levelvars[1,1..gml.Nrelevels],gml.modeltouses[1,j])
...
void merlin_init_ip_ltrunc(`gml' gml)
{
	
	if (sum(gml.adapt)) {
		gml.aghip_lt 		= asarray_create("real",2)
		gml.aghip2_lt 		= asarray_create("real",2)
		gml.aghlogl_lt		= asarray_create("real",1)	
		gml.stackednodes_lt	= asarray_create("real",1)
		gml.Li_ip_lt		= gml.Li_ip
	}
	...
	for (i=1;i<gml.Nlevels;i++) {
		...
		if (gml.usegh[i]) {
			...
			asarray(gml.stackednodes_lt,i,J(1,Nobs,asarray(gml.vcvs,i) * asarray(gml.baseGHnodes,i)))
					for (r=1;r<=gml.Nres[i];r++) {
						asarray(gml.aghip2_lt,(i,r),rowshape(asarray(gml.stackednodes_lt,i)[r,],Nobs))
					}	
					for (j=1; j<=Nobs; j++) {
						asarray(gml.aghip_lt,(i,j),asarray(gml.vcvs,i) * asarray(gml.baseGHnodes,i))
					}
					
					asarray(gml.aghlogl_lt,i,J(Nobs,1,merlin_lndmvnorm(asarray(gml.baseGHnodes,i)',I(gml.Nres[i]))') :+ log(sqrt(det(asarray(gml.vcvs,i)))))
