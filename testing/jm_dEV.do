...
survsim stime died, hazard(0.1:*1.2:*{t}:^0.2 :* 	///
	exp(0.2:*(u1 :+ (0.1:+u2):*{t} :+ 0.1:* {t}:^3 :+ 0.5:*trt) :+ ///
		0.1:*((0.1:+u2) :+ 0.3:*{t}:^2)	///
	)) cov(trt -0.5) maxt(5)
...
gen xb = u1 + (0.1+u2)*time + 0.1*time^3+ 0.5*trt
...
timer on 2
merlin 	(y trt fp(time, pow(1 3)) fp(time, pow(1))#M1[id1]@1 M2[id1]@1, family(gaussian) timevar(time)) ///
		(stime trt EV[y] dEV[y], family(weib, failure(died)) timevar(stime)) ,
timer off 2
timer list
